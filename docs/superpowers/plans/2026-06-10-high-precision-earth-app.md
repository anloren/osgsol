...
**关键路径前缀（全为绝对路径）：**
- 仓库根：`/Users/USER/osgverse`
- CORE 构建：`/Users/USER/osgverse/build/sdk_core`（产物）、`/Users/USER/osgverse/build/verse_core`（增量构建目录）
- 可执行：`/Users/USER/osgverse/build/sdk_core/bin/osgVerse_EarthExplorer`

**通用增量构建命令（每次改源码后用它，幂等增量）：**
```bash
cmake --build /Users/USER/osgverse/build/verse_core --target install --config Release
```

**通用截图验证命令（渲染 1500 帧给在线瓦片下载留时间，太阳朝相机以便看清，输出 /tmp/earth_capture_0.png）：**
```bash
cd /Users/USER/osgverse/build/sdk_core/bin && rm -f /tmp/earth_capture*.png && \
...
```bash
cd /Users/USER/osgverse && git add applications/earth_explorer/earth_main.cpp && \
...
```bash
cd /Users/USER/osgverse && git add applications/earth_explorer/EarthControlUI.h && \
...
```bash
chmod +x /Users/USER/osgverse/packaging/run.sh && \
echo "run.sh ready"
```
...
Run:
```bash
chmod +x /Users/USER/osgverse/packaging/package_macos.sh && \
/Users/USER/osgverse/packaging/package_macos.sh
```
...
Run:
```bash
otool -l /Users/USER/osgverse/dist/EarthExplorer.app/Contents/MacOS/osgVerse_EarthExplorer | grep -A2 LC_RPATH; \
echo "---one dylib---"; \
otool -l /Users/USER/osgverse/dist/EarthExplorer.app/Contents/lib/libosg.3.6.5.dylib | grep -A2 LC_RPATH
```
Expected: 可执行文件 rpath 含 `@executable_path/../lib`，dylib rpath 含 `@loader_path`，均不再出现 `$ORIGIN` 或 `/Users/USER/...` 绝对路径。

- [ ] **Step 5: 双击式启动验证（用 `open`，不依赖 DYLD 环境变量）**

Run（`open` 模拟 Finder 双击；随后等几秒截屏确认窗口起来，再关闭）：
```bash
open /Users/USER/osgverse/dist/EarthExplorer.app; sleep 12; \
...
```bash
open /Users/USER/osgverse/dist/EarthExplorer.app; sleep 10; \
...
```bash
cd /Users/USER/osgverse && git add packaging/run.sh packaging/package_macos.sh && \
...
Run:
```bash
open /Users/USER/osgverse/dist/EarthExplorer.app
```
...
```bash
cd /Users/USER/osgverse && git add tasks/lessons.md docs/ && \
