...
**通用命令：**
- 增量构建：`cmake --build /Users/USER/osgverse/build/verse_core --target install --config Release`
- 全球截图：
```bash
cd /Users/USER/osgverse/build/sdk_core/bin && rm -f /tmp/earth_capture_0.png && \
DYLD_LIBRARY_PATH=/Users/USER/osgverse/build/sdk_core/lib EARTH_AUTOCAP=1500 EARTH_SUN_TO_CAMERA=1 \
...
Run: `cmake --build /Users/USER/osgverse/build/verse_core --target install --config Release`
...
```bash
rm -rf ~/.osgverse_earth_cache; cd /Users/USER/osgverse/build/sdk_core/bin && rm -f /tmp/earth_capture_0.png && \
DYLD_LIBRARY_PATH=/Users/USER/osgverse/build/sdk_core/lib EARTH_AUTOCAP=1500 EARTH_FRAME_SLEEP_MS=20 EARTH_SUN_TO_CAMERA=1 \
...
```bash
cd /Users/USER/osgverse && git add readerwriter/UtilitiesEx.cpp && \
...
DYLD_LIBRARY_PATH=/Users/USER/osgverse/build/sdk_core/lib OSG_NOTIFY_LEVEL=NOTICE \
...
```bash
cd /Users/USER/osgverse && git add readerwriter/EarthManipulator.h readerwriter/EarthManipulator.cpp && \
...
#include <cstdio>
#include <cmath>
#include "/Users/USER/osgverse/readerwriter/SolarPosition.h"
int main()
{
...
```bash
cd /Users/USER/osgverse/build/sdk_core/bin && rm -f /tmp/earth_capture_0.png && \
DYLD_LIBRARY_PATH=/Users/USER/osgverse/build/sdk_core/lib EARTH_AUTOCAP=1500 EARTH_FRAME_SLEEP_MS=20 \
...
```bash
cd /Users/USER/osgverse && git add readerwriter/SolarPosition.h applications/earth_explorer/EarthControlUI.h && \
...
然后确认着色器已更新到运行目录：
```bash
cp /Users/USER/osgverse/assets/shaders/scattering_globe.frag.glsl /Users/USER/osgverse/build/sdk_core/shaders/
```
...
```bash
cd /Users/USER/osgverse && git add assets/shaders/scattering_globe.frag.glsl applications/earth_explorer/EarthControlUI.h && \
...
```bash
cd /Users/USER/osgverse/build/sdk_core/bin && rm -f /tmp/earth_capture_0.png && \
DYLD_LIBRARY_PATH=/Users/USER/osgverse/build/sdk_core/lib EARTH_AUTOCAP=2500 EARTH_FRAME_SLEEP_MS=30 EARTH_SUN_TO_CAMERA=1 \
...
```bash
cd /Users/USER/osgverse && git add readerwriter/TileCallback.cpp modeling/Math.cpp && \
...
Run: `/Users/USER/osgverse/packaging/package_macos.sh` （确认输出 `Built: .../dist/EarthExplorer.app`）

- [ ] **Step 4: Commit**

```bash
cd /Users/USER/osgverse && git add tasks/lessons.md docs/EarthExplorer.md && \
