...
Run:
```
grep -rn "define SHADER_DIR\|SHADER_DIR =" /Users/USER/osgverse --include=*.h --include=*.cpp | grep -v "build/\|dist/"
```
...
Run: `g++ -std=c++14 -fsyntax-only -x c++ /Users/USER/osgverse/applications/earth_explorer/LayerManager.h`
Expected: 无输出（通过）。
...
Run: `bash /Users/USER/osgverse/packaging/package_macos.sh`
