...
- [x] 起止年份、运行中取消和三个结果架槽均经过真实指针/键盘链路；
- [x] 抽屉/上下文帮助、事件定位和洞察标签关闭均经过真实点击与状态断言；
- [x] 回到全球、新闻来源/摘要以及照片/视频文件动作均经过正式命中区点击，
  自动化只截获外部打开边界并核对精确目标；
...
- UI 运行时：八个主模块、三档视口、科学菜单、滚轮双向、分析提交、
  ...
  结束年份、运行中取消、第二/第三结果架槽、两处模块帮助、事件定位及
  洞察标签关闭也完成真实命中验证；回到全球、新闻来源/摘要、照片/视频文件
  动作也点击正式命中区并核对相机、精确 URL、摘要合同和精确文件路径
