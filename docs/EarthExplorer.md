...
- **点击某个地震点**：面板弹出通用「要素详情」卡片（FeedLayer 框架统一提供，GDACS 等其它数据源共用同一卡位），显示震级、深度、位置、经纬、发生时间（相对）与「打开」按钮（跳转 USGS 事件页）；点「关闭」收起。

> 实现备注（T2）：地震层已从早期的专属 `quake_data.cpp` 迁移到通用 `feeds/usgs_quakes_feed.cpp`（FeedLayer 配置驱动框架），图层 id/显示名/env 钩子语义不变；专属的拾取自测钩子 `EARTH_QUAKE_PICKDBG` 已随迁移下线（通用拾取路径已由 GDACS 等其它数据源验证过）。
