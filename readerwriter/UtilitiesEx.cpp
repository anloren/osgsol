...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    // 持久磁盘瓦片缓存统计:disk 命中 / 网络实取。周期性 NOTICE + atexit 汇总
    // (析构期 osg::notify 的互斥锁可能已销毁会拉崩进程,退出汇总只能用 fprintf)
    static std::atomic<unsigned> sTileCacheHits(0), sTileCacheFetches(0);
    static void reportTileCacheStats()
    {
        fprintf(stderr, "[TileCache] summary: diskHits=%u, netFetches=%u\n",
                (unsigned)sTileCacheHits, (unsigned)sTileCacheFetches);
    }
    static std::string tileCacheDir();
    static void countTileCacheOp(std::atomic<unsigned>& counter)
    {
        ++counter;
        if (tileCacheDir().empty()) return;  // 缓存关闭时只计数不打日志
        unsigned total = (unsigned)sTileCacheHits + (unsigned)sTileCacheFetches;
        if (total % 200 == 0)
        {
            OSG_NOTICE << "[TileCache] diskHits=" << (unsigned)sTileCacheHits
                       << ", netFetches=" << (unsigned)sTileCacheFetches << std::endl;
        }
    }

    // 缓存根目录(整个进程只解析一次):
    //   EARTH_TILE_CACHE 未设 → $HOME/Library/Caches/EarthExplorer(运行时展开 HOME
    //     成绝对路径;打包 .app 双击启动 cwd=/,禁用相对路径);
    //   EARTH_TILE_CACHE=0   → 返回空串 = 彻底关闭磁盘缓存(纯网络+进程内缓存);
    //   其它非空值           → 自定义缓存根目录。
    // 旧默认目录 ~/.osgverse_earth_cache(38a4c93d 起)首次启动整目录 rename 原子迁移
    // 到新默认位置,已缓存瓦片不作废(否则遇 Google 软限流又是数分钟橙色冷启动)。
    static std::string tileCacheDir()
    {
        static std::string sDir = []() -> std::string {
            ...
            if (env && env[0])
            {
                if (strcmp(env, "0") == 0) return std::string();  // 显式关闭:不建目录、不注册统计
                dir = env;
            }
            else
            {
                // 默认路径按 macOS 惯例($HOME/Library/Caches);Linux 会得到同一 mac 风格
                // 路径(可用但非 XDG 惯例)、Windows 通常无 HOME → 默认禁用——本产品当前只
                // 打包 macOS .app,跨平台惯例路径留待有需求时再做。
                const char* home = getenv("HOME");
                if (home == NULL || home[0] == '\0') return std::string();  // 无 HOME 不猜相对路径
                dir = std::string(home) + "/Library/Caches/EarthExplorer";
                std::string legacy = std::string(home) + "/.osgverse_earth_cache";
                if (!osgDB::fileExists(dir) && osgDB::fileExists(legacy))
                {
                    // 迁移失败(EXDEV/权限/父目录缺失)不致命:旧目录原地保留,下面照常建新根,
                    // 只是已缓存瓦片不再命中——必须留痕,否则"缓存凭空清零"无从排查。
                    if (::rename(legacy.c_str(), dir.c_str()) != 0)
                    {
                        OSG_WARN << "[TileCache] failed to migrate legacy cache " << legacy
                                 << " -> " << dir << " (errno=" << errno
            ...
            {
                OSG_WARN << "[TileCache] failed to create disk cache root " << dir
                         << ", disk tile cache disabled" << std::endl;
                return std::string();
            }
            OSG_NOTICE << "[TileCache] disk cache at " << dir << std::endl;
            atexit(reportTileCacheStats);  // 仅在磁盘缓存真正启用时注册退出汇总
            return dir;
        }();
        return sDir;
    }
    ...
    {
        std::string dir = tileCacheDir();
        if (dir.empty()) return std::string();  // 磁盘缓存已关闭
        std::hash<std::string> h; char buf[32];
        snprintf(buf, sizeof(buf), "%016llx", (unsigned long long)h(url));
        // 两级布局:哈希前 2 字符做扇出子目录(至多 256 个),避免十几万瓦片挤一个目录
        std::string fanned = dir + "/" + std::string(buf, 2) + "/" + std::string(buf) + ".tile";
        if (osgDB::fileExists(fanned)) return fanned;
        // 迁移前写入的旧扁平布局兜底:命中则沿用旧文件;未命中的新写入一律走扇出布局
        std::string flat = dir + "/" + std::string(buf) + ".tile";
        if (osgDB::fileExists(flat)) return flat;
        return fanned;
    }
                                            ...
                                            const std::vector<std::string>& reqHeaders)
    {
        ...
        if (!scheme.empty())
        {
...
#else
            std::string cf = tileCacheFile(url);  // 空串 = 磁盘缓存已关闭(EARTH_TILE_CACHE=0)
            if (!cf.empty())
            {
            ...
            else
            {
                ...
                countTileCacheOp(sTileCacheFetches);
            }
            ...
            if (!buffer.empty() && !cf.empty())  // cf 空 = 磁盘缓存已关闭,只取不存
            {
                osgDB::makeDirectory(osgDB::getFilePath(cf));  // 扇出子目录按需创建
