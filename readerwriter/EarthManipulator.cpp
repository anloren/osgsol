...
void EarthManipulator::updateTerrainFloor()
{
    ...
    // must never sink to or below the ellipsoid datum. Below the datum the eye is inside/under the
    // crust, so the near-side geometry no longer depth-occludes the far hemisphere and the camera
    ...
    if (!_hasTerrainProbe && hEye < kHardSeaFloor)
    {
        ...
        if (s_hardFloorDebug)
        {
            OSG_NOTICE << "[hardfloor] TRIGGER hEye=" << hEye << " hardLift=" << hardLift
                       << " desiredLift=" << desiredLift << "\n";
        }
    }
