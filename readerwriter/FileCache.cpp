...
osgDB::ReaderWriter* FileCache::getReaderWriter(const std::string& fileName) const
{
    ...
    else const_cast<FileCache*>(this)->_cachedReaderWriters[ext] = rw;
    // 诊断钩子(EARTH_IMG_DEBUG=1):记录扩展名首次解析到哪个图像插件。历史上 jpg/png
    // 由 stb(RGB)还是 imageio(BGRA)解码是进程级竞态,曾疑与"瓦片 R/B 对调"相关;
    // 复发时开此环境变量可立即定位每类扩展名的实际解码器。默认关闭零开销(仅一次 getenv)。
    if (rw != NULL && getenv("EARTH_IMG_DEBUG") != NULL)
    { OSG_NOTICE << "[IMGDBG] FileCache ext=" << ext << " -> " << rw->className() << std::endl; }
    return rw;
}
