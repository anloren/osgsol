...
namespace osgVerse
{
    ...
    public:
        ...
        // Query real terrain altitude (ellipsoid height, meters) at an arbitrary lat/lon
        // (radians), via a vertical scene-intersection probe against the rendered terrain.
        // Was manipulator-internal only (used by updateTerrainFloor's ground-following);
        // promoted to public so other main-thread-only consumers (e.g. earth_explorer's
        // feed_layer.cpp, correcting marker lift from ellipsoid-relative to terrain-relative
        // in high-elevation regions) can reuse the exact same query instead of duplicating
        // the intersection logic. Still main-thread-only: does an IntersectionVisitor
        // traversal against _viewer->getCamera(), same constraint as the caller already has.
        // Returns false if no intersection found (e.g. terrain tile not loaded yet) — callers
        // must handle that by falling back to their prior/default height, never crash/assume.
    ...
    protected:
        ...
        // terrainAltitudeAt() declaration moved to the public section above (now reused
        // outside the manipulator); definition in EarthManipulator.cpp is unchanged.
    ...
    protected:
                                       ...
                                       double& zfar, value_type near_far_ratio) const
        {
            ...
            {
                // Defensive fallback (double safety). The eye has reached or sunk below the
                // earth radius — the hard sea-level floor in updateTerrainFloor() should make
                // this unreachable in practice. The original code fell through to `return false`
                // here, which leaves OSG's default far plane wide open, so the camera saw
                // straight through the globe to the backside of the crust (user report: standing
                // in Fujian and seeing South America ~12742km away). Instead pin a converged
                // near/far small enough that nothing can reach the opposite shell. The happy path
                // above (distance > rp) is left byte-for-byte unchanged.
                static const bool s_hardFloorDebug = (getenv("EARTH_HARDFLOOR_DEBUG") != NULL);
                if (s_hardFloorDebug)
                {
                    fprintf(stderr, "[hardfloor] NEARFAR FALLBACK distance=%.3f rp=%.3f "
                            "(eye at/below earth radius)\n", distance, rp);
                }

                double temp_zfar = zfar;
                znear = 1.0;
                zfar = osg::minimum(50000.0, temp_zfar);
                if (zfar < znear + epsilon) zfar = znear + epsilon;
