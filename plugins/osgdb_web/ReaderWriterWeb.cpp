...
public:
                                ...
                                const osgDB::Options* options) const
    {
        ...
        if (cache && cache->existsInCache(fileName))
        {
            ...
            {
                // 诊断钩子(EARTH_IMG_DEBUG=1):记录缓存命中解码结果的像素格式(瓦片 R/B 对调复发排查用)
                if (getenv("EARTH_IMG_DEBUG") != NULL && cacheResult.validImage())
                {
                    osg::Image* img = cacheResult.getImage();
                    const unsigned char* px = img->data();
                    OSG_NOTICE << "[IMGDBG] CACHE " << fileName << " fmt=0x" << std::hex
                               << img->getPixelFormat() << std::dec << " px0=" << (int)px[0]
                               << "," << (int)px[1] << "," << (int)px[2] << std::endl;
                }
                return cacheResult;
            }
        }
        ...
        // 诊断钩子(EARTH_IMG_DEBUG=1):记录本次解码用的插件与像素格式(fmt=0x1907 RGB=stb,
        // 0x80e1 BGRA=imageio;瓦片 R/B 对调复发排查用)
        if (getenv("EARTH_IMG_DEBUG") != NULL && readResult.validImage())
        {
            osg::Image* img = readResult.getImage();
            const unsigned char* px = img->data();
            OSG_NOTICE << "[IMGDBG] NET " << fileName << " reader=" << reader->className()
                       << " fmt=0x" << std::hex << img->getPixelFormat() << std::dec
                       << " px0=" << (int)px[0] << "," << (int)px[1] << "," << (int)px[2] << std::endl;
        }
...
protected:
    ...
    osgDB::ReaderWriter* getReaderWriter(const std::string& extOrMime, bool isExt) const
    {
        ...
        if (rw) const_cast<ReaderWriterWeb*>(this)->_cachedReaderWriters[extOrMime] = rw;
        // 诊断钩子(EARTH_IMG_DEBUG=1):记录扩展名/MIME 首次解析到哪个插件(瓦片 R/B 对调复发排查用)
        if (rw != NULL && getenv("EARTH_IMG_DEBUG") != NULL)
        { OSG_NOTICE << "[IMGDBG] WebRW key=" << extOrMime << " -> " << rw->className() << std::endl; }
        return rw;
    }
