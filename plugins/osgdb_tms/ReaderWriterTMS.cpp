...
#include "TmsLayerLoadPool.h"
#include "TmsOverlaySelection.h"

// 常驻 keep-alive 线程池:并行拉取一块瓦片的多层。worker 复用 loadFileData 的 thread_local
// 长连接(临时线程会每次重握 TLS、反更慢)。线程由插件内的 Meyers 对象拥有；插件 dlclose
// 前析构会唤醒并 join 全部 worker，禁止残留线程返回已经卸载的 osgdb_tms 代码。
// EARTH_TILE_POOL:worker 数(默认 8,0=关)。
...
protected:
                          ...
                          const Options* opt, bool useWM, bool flatten) const
    {
        ...
        osgVerse::TmsLayerLoadPool::instance().runAll(layerTasks);
