...
public:
                                   ...
                                   const Options* options) const
    {
        ...
        // stb 按"首行=图片顶行"写文件,而 OSG 惯例(glReadPixels 读回、本插件 readImage
        // flipVertical 之后)是 BOTTOM_LEFT 起始 → 写前需翻转,否则输出上下颠倒。此前漏翻,
        // 与 osgdb_imageio(写前正确翻转)形成两写入器约定不一致:哪个插件先注册谁接管
        // "png" 扩展名(多线程分页加载下先后不定),同一命令的截图/快照随机倒置。
        // 本插件 readImage 读入时 flipVertical 成 BOTTOM_LEFT,写出时对称翻回,读写自洽;
        // origin 已是 TOP_LEFT 的图像保持原样直写。
        int r = 0, w = image.s(), h = image.t(), comp = osg::Image::computeNumComponents(image.getPixelFormat());
        if (image.getDataType() == GL_UNSIGNED_BYTE)
        {
            // 先判扩展名再深拷贝翻转:不支持的格式(psd/hdr 等)不必白付一次整图拷贝
            if (ext != "png" && ext != "jpg") return WriteResult::NOT_IMPLEMENTED;
            const unsigned char* pixels = image.data();
            int rowStep = image.getRowStepInBytes();
            osg::ref_ptr<osg::Image> flipped;
            if (image.getOrigin() == osg::Image::BOTTOM_LEFT)
            {
                flipped = new osg::Image(image, osg::CopyOp::DEEP_COPY_IMAGES);
                flipped->flipVertical();
                pixels = flipped->data(); rowStep = flipped->getRowStepInBytes();
            }
            if (ext == "png")
                r = stbi_write_png(fileName.c_str(), w, h, comp, pixels, rowStep);
            else if (ext == "jpg")
                r = stbi_write_jpg(fileName.c_str(), w, h, comp, pixels, 90);
